...
def store_weather_data(city, temp, feels_like, weather_condition, timestamp):
    query = """
    INSERT INTO weather_data (city, temp, feels_like, weather_condition, timestamp)
    VALUES (%s, %s, %s, %s, %s)
    """
    cursor.execute(query, (city, temp, feels_like, weather_condition, timestamp))
    conn.commit()
...
def calculate_daily_summary():
    for city in CITIES:
        ...
        avg_temp, max_temp, min_temp, dominant_condition = result[0]
        ...
        if dominant_condition is not None:
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(query_summary, (city, today, avg_temp, max_temp, min_temp, dominant_condition))
            conn.commit()
